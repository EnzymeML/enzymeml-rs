...
impl EnzymeMLDocument {
    /// Converts the EnzymeML document to an SBML representation within a COMBINE archive.
    ///
    /// This method provides a convenient interface for exporting EnzymeML documents to the
    /// standardized SBML format within a COMBINE archive structure. The conversion process
    /// maintains complete fidelity of biochemical data while ensuring compatibility with
    /// SBML-based modeling tools and workflows.
    ///
    /// # Process Overview
    ///
    /// The conversion follows a structured approach:
    /// 1. **Model Structure**: Creates SBML document framework with proper namespace declarations
    /// 2. **Entity Mapping**: Converts EnzymeML entities to corresponding SBML components
    /// 3. **Annotation Embedding**: Embeds version-specific EnzymeML metadata as SBML annotations
    /// 4. **Data Serialization**: Exports measurement data as CSV files within the archive
    /// 5. **Archive Assembly**: Packages all components into a standards-compliant COMBINE archive
    ///
    /// # Data Preservation
    ///
    /// All EnzymeML-specific information is preserved through:
    /// - **SBML Annotations**: Structured metadata embedded as XML annotations
    /// - **SBO Terms**: Semantic annotations for biological entity classification
    /// - **Unit Definitions**: Comprehensive unit system mapping and preservation
    /// - **Custom Properties**: Extension data maintained through annotation schemas
    ///
    /// # Version Compatibility
    ///
    /// The method supports both EnzymeML format versions:
    /// - **V1**: Legacy format with reaction-centric data organization
    /// - **V2**: Modern format with enhanced metadata and unified structure
    ///
    /// Version selection affects:
    /// - Annotation schema structure and content organization
    /// - Measurement data file naming and internal organization
    /// - Metadata field availability and semantic interpretation
    ///
    /// # Arguments
    ///
    /// * `version` - Target EnzymeML version determining annotation format and data organization
    ///
    /// # Returns
    ///
    /// * `Ok(CombineArchive)` - Complete COMBINE archive ready for export or further processing
    /// * `Err(SBMLError)` - Detailed error information for conversion failures
    ///
    /// # Error Conditions
    ///
    /// Conversion may fail due to:
    /// - **Data Inconsistencies**: Invalid references between model components
    /// - **Unit Conflicts**: Incompatible or undefined unit definitions
    /// - **Serialization Issues**: Problems during SBML XML generation or CSV data export
    /// - **Archive Creation**: Filesystem or memory constraints during archive assembly
    ///
    /// # Usage Examples
    ///
    /// ```ignore
    /// use rust_enzyme_ml::(EnzymeMLDocument, sbml::version::EnzymeMLVersion);
    ///
    /// // Convert to EnzymeML v2 format
    /// let archive = document.to_sbml(&EnzymeMLVersion::V2)?;
    /// archive.save("model.omex")?;
    ///
    /// // Convert to legacy v1 format for compatibility
    /// let legacy_archive = document.to_sbml(&EnzymeMLVersion::V1)?;
    /// legacy_archive.save("legacy_model.omex")?;
    /// ```
    ///
    /// # Archive Structure
    ///
    /// The resulting COMBINE archive contains:
    ///
    /// Version 1:
    /// - `model.xml`: Primary SBML document with complete model structure
    /// - `data/*.csv`: Measurement data files with time-series experimental data
    /// - `manifest.xml`: Archive manifest automatically generated by COMBINE library
    ///
    /// Version 2:
    /// - `model.xml`: Primary SBML document with complete model structure
    /// - `data.tsv`: Measurement data file with time-series experimental data
    /// - `manifest.xml`: Archive manifest automatically generated by COMBINE library
    ///
    /// # Standards Compliance
    ///
    /// Generated archives conform to:
    /// - **SBML Level 3**: Modern SBML specification with extension support
    /// - **COMBINE Archive**: Standardized format for systems biology model distribution
    /// - **EnzymeML Schema**: Version-specific annotation schemas for biochemical metadata
    /// - **SBO Ontology**: Semantic annotation standards for biological entity classification
    pub fn to_sbml(
        self,
        version: impl AsRef<EnzymeMLVersion>,
    ) -> Result<CombineArchive, SBMLError> {
        to_omex(&self, version)
    }
}
...
pub fn to_omex(
    ...
    let sbml_doc = to_sbml(enzmldoc, version.as_ref())?;
    archive.add_entry(
        "./model.xml",
        KnownFormats::SBML,
        true,
    ...
    write_measurement_data(enzmldoc, &mut archive, version.as_ref())?;
